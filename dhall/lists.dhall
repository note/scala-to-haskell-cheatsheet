...
let mapListHaskellQuoted1 =
      F.quoteExplanation "fmap :: Functor f => (a -> b) -> f a -> f b"
...
let mapListHaskell =
      [ hask
          ...
          ( Some
              "${F.code
                   "map"} is defined only for list: ${mapListHaskellQuoted0}"
          )
      , hask
          "Haskell"
          mapListHaskell1
          ( Some
              "${F.code
                   "fmap"} is defined in terms of Functor: ${mapListHaskellQuoted1}"
          )
      , hask
          "Haskell"
          mapListHaskell2
          (Some "${F.code "<\$>"} is an infix operator for ${F.code "fmap"}")
      ]
